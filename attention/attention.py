...
class AttentionLayer(torch.nn.Module):
    def __init__(self, config):
        ...
        self.dim, self.num_head = config.dim, config.num_multihead
    ...
    def _divideHead(self, x):  # 多头注意力实现
        ...
        x.view(batch, num_token, self.num_head, -1)
    ...
    def forword(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        weight = torch.softmax(weight, dim=-1)  # 对最后一个维度的每一行分别归一化
...
class TransformerBlock(nn.module):
    def __init__(self, config):
        ...
        self.layrNorm1 = layerNorm(self.hidden_size, config.norm_eps)

        self.layrNorm2 = layerNorm(self.hidden_size, config.norm_eps)
